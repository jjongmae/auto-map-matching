...
| **노이즈 강건성** | 보통 | 강함 | 보통 |
| **불연속 함수** | 강함 | 강함 | **취약** |
| **수렴 속도** | 보통 | 느림 | 빠름 (smooth 함수일 때) |
| **정확도** | 보통 | 보통 | 높음 (smooth 함수일 때) |
| **권장 파라미터 수** | 제한 없음 | 10개 이하 | 제한 없음 |
| **로봇 비전 사용** | 드묾 | 가끔 | 표준 (smooth 잔차일 때) |
...
> **현재 프로젝트**: `min()` 기반 비용 함수 → non-smooth → **Powell 또는 Nelder-Mead 권장**
...
1. **Powell**: 가장 먼저 시도 (경계 지원, 불연속 함수에 강건)
2. **Nelder-Mead**: Powell이 실패하거나 노이즈가 의심될 때
3. **LM**: 참고용 (이론적으로는 좋지만 현재 비용 함수와 궁합이 맞지 않음)
